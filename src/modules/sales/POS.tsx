...
export default function POS({ products, categories, customers, settings, onAddInvoice, onUpdateProductStock, onAddCustomer }: POSProps) {
  ...
  // Calculations - Memoized to prevent recalculation on unrelated state changes (modal toggles, timer, query etc.)
  const { subtotal, totalDiscount, taxableAmount, taxRate, taxAmount, grandTotal } = useMemo(() => {
  ...
  // Filter products by category and search - Memoized to prevent O(N) array filtering on unrelated state changes (e.g. cart/totals, payments, modals)
