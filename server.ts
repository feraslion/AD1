...
async function authenticate(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!userRecord) {
      return sendError(res, 'غير مصرح به - يلزم تسجيل الدخول بحساب فعلي', null, 401);
    }
...
// New deployments start with an empty database. Business data, accounts, users,
// roles and settings are created explicitly by the organization after installation.
async function seedDefaultData() {
  try {
    await ensureDatabaseTables();
    console.log('Database schema is ready. No default or demo records were created.');
  } catch (error) {
