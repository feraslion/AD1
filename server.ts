...
import searchRoutes from './src/core/server/routes/v1/search.routes.ts';
...
function sendResponse(res: Response, data: unknown, status = 200, pagination?: unknown) {
...
function sendError(res: Response, message: string, details?: unknown, status = 500) {
  res.status(status).json({
    success: false,
    error: message,
    ...(details && { details })
...
async function authenticate(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    // Default manager user fallback for dev/testing session (strictly non-production only)
...
async function safeSeedStep(stepName: string, task: () => Promise<void>) {
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (attempt === 3) {
        console.error(`[Seeder] Error seeding ${stepName} after 3 attempts:`, err?.message || err);
      } else {
...
async function seedDefaultData() {
  try {
    ...
    await safeSeedStep('Company', async () => {
    ...
    await safeSeedStep('Branch', async () => {
    ...
    await safeSeedStep('Accounts', async () => {
    ...
    await safeSeedStep('Suppliers', async () => {
    ...
    await safeSeedStep('Units', async () => {
    ...
    await safeSeedStep('Categories', async () => {
    ...
    await safeSeedStep('Roles', async () => {
    ...
    await safeSeedStep('Permissions', async () => {
    ...
    await safeSeedStep('RolePermissions', async () => {
    ...
    await safeSeedStep('Users', async () => {
    ...
    await safeSeedStep('Cashboxes', async () => {
    ...
    await safeSeedStep('PostingRules', async () => {
    ...
    await safeSeedStep('Currencies', async () => {
