...
## 2025-03-05 - [Optimize Invoice Line Item Parent-Child Array Filtering]
**Learning:** Filtering line items inside an array map function (`allItems.filter(item => item.invoiceId === inv.id)`) creates an $O(N \times M)$ nested loop performance bottleneck when querying lists of invoices.
**Action:** Pre-group child items into an in-memory `Map` keyed by parent ID before iterating, reducing total complexity to $O(N + M)$ with $O(1)$ lookup time per invoice.
