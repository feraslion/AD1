...
export class Supplier {
  ...
  // Validation
  public static validate(props: SupplierProps): void {
    const errors: string[] = [];
    if (!props.id || !props.id.trim()) errors.push('Supplier ID is required');
    if (!props.name || !props.name.trim()) errors.push('Supplier name is required');
    if (props.taxNumber && !/^\d{15}$/.test(props.taxNumber.trim())) {
      errors.push('Supplier VAT Tax Number must be 15 digits');
    }
    if (errors.length > 0) throw new ValidationError(errors);
  }

  // Business Rules
  public recordPurchaseCredit(amount: Money): void {
    if (this._status === 'inactive') {
      throw new BusinessRuleError('Cannot create purchase from an inactive supplier', 'SUPPLIER_INACTIVE');
    }
    this._balance = this._balance.add(amount);
    this._updatedAt = new Date();
    DomainEventPublisher.getInstance().publish(createDomainEvent('SupplierBalanceUpdated', this.id, { newBalance: this._balance.amount, type: 'credit' }));
  }

  public recordPaymentDebit(amount: Money): void {
    this._balance = this._balance.subtract(amount);
    this._updatedAt = new Date();
    DomainEventPublisher.getInstance().publish(createDomainEvent('SupplierBalanceUpdated', this.id, { newBalance: this._balance.amount, type: 'debit' }));
  }
}
