...
router.get('/workflows/rules', authorize(['manager', 'manage_users']), (req, res) => {
...
router.get('/workflows/approvals', authorize(['manager']), (req, res) => {
  const approvals = WorkflowRepository.getAllApprovals();
  res.json({ success: true, data: approvals });
});

router.post('/workflows/approvals/:id/approve', authorize(['manager']), async (req, res) => {
  try {
...
router.post('/workflows/approvals/:id/reject', authorize(['manager']), async (req, res) => {
  try {
    const { approverName, notes } = req.body;
...
// 2. Audit Trail
router.get('/audit-logs', authorize(['manager']), async (req, res) => {
...
// 3. Notifications
router.get('/notifications', authorize(['manager', 'cashier', 'accountant', 'inventory']), async (req, res) => {
...
router.post('/notifications/read-all', authorize(['manager', 'cashier', 'accountant', 'inventory']), (req, res) => {
